...
# useful modules
from .build_nets.configure_utils import dict2config
from .build_nets.SharedUtils import change_key
from .build_nets.cell_searchs import CellStructure, CellArchitectures
...
def get_cell_based_tiny_net(config):
    ...
    if super_type == "basic" and config.name in group_names:
        from .build_nets.cell_searchs import nas201_super_nets as nas_super_nets
        ...
        from .build_nets.cell_searchs import nasnet_super_nets as nas_super_nets
        ...
        from .build_nets.cell_infers import TinyNetwork
        ...
        from .build_nets.shape_infers import DynamicShapeTinyNet
        ...
        from .build_nets.cell_infers import NASNetonCIFAR
...
def get_search_spaces(xtype, name) -> List[Text]:
    if xtype == "cell":
        from .build_nets.cell_operations import SearchSpaceNames
...
def get_cifar_models(config, extra_path=None):
    super_type = getattr(config, "super_type", "basic")
    if super_type == "basic":
        from .build_nets.CifarResNet import CifarResNet
        from .build_nets.CifarDenseNet import DenseNet
        from .build_nets.CifarWideResNet import CifarWideResNet
        ...
        from .build_nets.shape_infers import InferWidthCifarResNet
        from .build_nets.shape_infers import InferDepthCifarResNet
        from .build_nets.shape_infers import InferCifarResNet
        from .build_nets.cell_infers import NASNetonCIFAR
...
def get_imagenet_models(config):
    super_type = getattr(config, "super_type", "basic")
    if super_type == "basic":
        from .build_nets.ImageNet_ResNet import ResNet
        from .build_nets.ImageNet_MobileNetV2 import MobileNetV2
        ...
        infer_mode = super_type.split("-")[1]
        if infer_mode == "shape":
            from .build_nets.shape_infers import InferImagenetResNet
            from .build_nets.shape_infers import InferMobileNetV2
...
def obtain_search_model(config):
    if config.dataset == "cifar":
        if config.arch == "resnet":
            from .build_nets.shape_searchs import SearchWidthCifarResNet
            from .build_nets.shape_searchs import SearchDepthCifarResNet
            from .build_nets.shape_searchs import SearchShapeCifarResNet
            ...
            from .build_nets.shape_searchs import SearchWidthSimResNet
        ...
        from .build_nets.shape_searchs import SearchShapeImagenetResNet
