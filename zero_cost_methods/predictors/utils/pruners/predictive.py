...
def find_measures(
    ...
    for k, v in measures_arr.items():
        if k == "jacov" or k == 'zenscore':
            measure_score = v
        elif k == 'synflow':
            measure_score = 0.0
            for grad_abs in v:
                if len(grad_abs.shape) == 4:
                    measure_score += float(torch.mean(torch.sum(grad_abs, dim=[1, 2, 3])))
                elif len(grad_abs.shape) == 2:
                    measure_score += float(torch.mean(torch.sum(grad_abs, dim=[1])))
                else:
                    raise RuntimeError('!!!')
        else:
            measure_score = sum_arr(v)
        measure_score_list[k] = measure_score
    return measure_score_list