...
# from zero_cost_methods.utils_2.utils_2 import get_project_root
...
def get_nasbench101_api(dataset=None):
    ...
    nb101_data = api.NASBench(
        os.path.join('/content/drive/MyDrive/Research', "nasbench_only108.tfrecord")
    )
...
def get_nasbench201_api(dataset=None):
    ...
    if dataset == "cifar10":
        with open(
            os.path.join(
                '/content/drive/MyDrive/Research', "nb201_cifar10_full_training.pickle"
            ),
            "rb",
        ) as f:
            data = pickle.load(f)
        ...
        with open(
            os.path.join(
                '/content/drive/MyDrive/Research',  "nb201_cifar100_full_training.pickle"
            ),
            "rb",
        ) as f:
            data = pickle.load(f)
        ...
        with open(
            os.path.join(
                '/content/drive/MyDrive/Research',  "nb201_ImageNet16_full_training.pickle"
            ),
            "rb",
        ) as f:
            data = pickle.load(f)
